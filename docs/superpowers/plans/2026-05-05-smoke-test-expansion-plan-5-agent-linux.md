# Smoke Test Expansion — Plan 5: test_agent_live.py (Linux × 3 clouds)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `backend/tests/smoke/test_agent_live.py` with the AWS Linux agent track fully implemented and stub tracks for GCP and Azure Linux. The AWS track spins up an EC2 instance, deploys the agent, and verifies all Linux agent command groups via SSM shell commands (same proven pattern as Phase B). GCP and Azure Linux tracks are stubs pending Sub-project B completion.

**Architecture:** Single file, independently runnable. Reuses `smoke_helpers.py` infrastructure (NexplaneClient, cloud SDK helpers, make_base_parser). The `--cloud` flag (aws/gcp/azure/all) and `--os` flag (linux/windows/all) select which tracks to run. Only the AWS Linux track is implemented — GCP and Azure tracks print STUB messages.

**Tech Stack:** Python 3.12, boto3, Nexplane CR machinery, smoke_helpers
...
### Task 1: Create `test_agent_live.py`

**Files:**
- Create: `backend/tests/smoke/test_agent_live.py`

- [ ] **Step 1: Create the file**

Create `backend/tests/smoke/test_agent_live.py` with this complete content:
...
def _setup_aws_linux_instance(client: NexplaneClient, cloud_account_id: str,
    ...
    instance_name = f"nexplane-agent-smoke-linux-aws"
...
def run_ossecurity_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """ossecurity: configure_selinux, configure_apparmor (skip), configure_seccomp,
    apply_sysctl_hardening, configure_host_firewall, blacklist_kernel_modules,
    harden_mount_options, deploy_auditd_rules, setup_file_integrity_monitoring,
    audit_os_security_posture, audit_ebpf_posture, configure_ebpf_security_policy,
    deploy_ebpf_policy."""
    print("\n  [ossecurity]")
...
def run_linuxauth_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """linuxauth: harden_ssh, configure_pam, manage_ca_certificates, configure_ntp,
    audit_users_and_groups, audit_privesc_vulnerabilities."""
    print("\n  [linuxauth]")
...
def run_crossplatform_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """crossplatform: harden_tls_protocols, configure_dns_resolver, audit_software_inventory,
    configure_syslog."""
    print("\n  [crossplatform]")
...
def run_reboot_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """reboot: graceful_reboot (skip actual reboot — verify the mechanism is present),
    verify_post_reboot."""
    print("\n  [reboot]")
...
def run_iac_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """iac: terraform_plan (plan-only, non-destructive)."""
    print("\n  [iac]")
...
def run_linuxupgrade_aws(client: NexplaneClient, instance_asset_id: str, instance_id: str) -> None:
    """linuxupgrade: estimate_image_size (non-destructive check only)."""
    print("\n  [linuxupgrade]")
...
- [ ] **Step 3: Verify stub tracks run (no credentials needed)**
...
docker exec nexplane-backend-1 python tests/smoke/test_agent_live.py \
  ...
  --cloud all --os all \
  --phases linux_patch,ossecurity 2>&1 | grep -E "(STUB|Track:|✅)"
```

Expected: prints "STUB" messages for GCP/Azure Linux and all Windows tracks, then `✅ ALL SELECTED TRACKS PASSED`... but will fail at AWS Linux track since it tries to provision. Instead test with gcp+azure only:
...
docker exec nexplane-backend-1 python tests/smoke/test_agent_live.py \
  ...
  --cloud gcp --os all \
  --phases linux_patch \
  --gcp-project nexplane 2>&1 | tail -8
```

Expected: GCP Linux STUB + GCP Windows STUB + `✅ ALL SELECTED TRACKS PASSED`
...
git commit -m "feat(smoke): create test_agent_live.py — AWS Linux track + stub tracks for GCP/Azure/Windows"
```

---

**Plan 5 complete.** Plan 6 adds:
- AWS/GCP/Azure Windows tracks (win_patch + winharden)
- README update reflecting multi-cloud test coverage
