...
def get_ssm_role(iam) -> str | None:
    """Return the name of a role that includes AmazonSSMManagedInstanceCore."""
    try:
...
def launch_runner(ec2, iam, key_name: str | None = None) -> str:
    ...
    _, subnet_id = get_default_vpc_subnet(ec2)
    ...
    ssm_role = get_ssm_role(iam)
    if ssm_role:
        launch_kwargs["IamInstanceProfile"] = {"Name": ssm_role}
    else:
        print("  ⚠️  No SSM-capable IAM role found — SSM commands may fail")
...
""",
    ...
    try:
        ...
        test_cmd_parts = [
            ...
            f"python3 smoke/test_aws_live.py"
