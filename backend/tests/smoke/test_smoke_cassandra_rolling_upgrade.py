...
""").decode()
    ...
    log(f"  Waiting for Cassandra port 9042 on {private_ip} (up to 30 min — booting from AMI)")
    ...
    pytest.fail(f"Cassandra never reachable on {private_ip}:9042 within 30 min (launch from AMI)")
