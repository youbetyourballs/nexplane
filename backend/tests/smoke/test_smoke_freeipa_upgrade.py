...
""").decode()
    ...
    log(f"  Waiting for LDAP port 389 on {private_ip} (up to 15 min)")
    ...
    pytest.fail(f"FreeIPA LDAP never reachable on {private_ip}:389 within 15 min")
    ...
    for run in runs:
        if run.get("status") in ("rolled_back", "rollback_failed"):
            ...
            if "rolled_back" in r or "data_loss_warning" in r:
                return r
    # Fallback: check top-level rollback_result
