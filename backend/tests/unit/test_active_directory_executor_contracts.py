...
for pkg in [
    ...
    "oci", "ldap3", "ldap3.core", "winrm", "winrm.protocol",
...
EXECUTORS_DIR = pathlib.Path(__file__).parent.parent.parent / "app" / "connectors" / "executors" / "active_directory"
...
def test_rollback_capability_declared(name):
    mod = importlib.import_module(f"app.connectors.executors.active_directory.{name}")
