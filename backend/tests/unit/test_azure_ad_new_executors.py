...
async def test_discover_users_returns_users():
    from app.connectors.executors.azure_ad.discover_users import execute
    ...
    with patch("app.connectors.executors.azure_ad.discover_users.get_azure_ad_client", return_value=mock_client):
...
async def test_discover_users_no_creds_skipped():
    from app.connectors.executors.azure_ad.discover_users import execute
    with patch("app.connectors.executors.azure_ad.discover_users.get_azure_ad_client", return_value=None):
...
async def test_get_group_membership_returns_groups():
    from app.connectors.executors.azure_ad.get_group_membership import execute
    mock_client = AsyncMock()
    mock_client.get_group_membership = AsyncMock(return_value=[
        {"id": "g1", "displayName": "Security Team"}
    ])
    with patch("app.connectors.executors.azure_ad.get_group_membership.get_azure_ad_client", return_value=mock_client):
...
async def test_get_group_membership_no_creds_skipped():
    from app.connectors.executors.azure_ad.get_group_membership import execute
    with patch("app.connectors.executors.azure_ad.get_group_membership.get_azure_ad_client", return_value=None):
