...
for pkg in [
    "boto3", "botocore", "google.cloud", "google.cloud.compute_v1",
    "google.cloud.storage", "google.cloud.dns", "google.auth",
...
EXECUTORS_DIR = pathlib.Path(__file__).parent.parent.parent / "app" / "connectors" / "executors" / "gcp"
...
def test_rollback_capability_declared(name):
    mod = importlib.import_module(f"app.connectors.executors.gcp.{name}")
