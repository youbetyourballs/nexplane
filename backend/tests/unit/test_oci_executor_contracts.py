...
for pkg in [
    ...
    "google.cloud", "google.auth", "oci", "oci.config", "oci.core",
    "oci.identity", "oci.database", "oci.object_storage",
    "ldap3", "winrm", "kubernetes", "requests", "httpx",
]:
    parts = pkg.split(".")
...
EXECUTORS_DIR = pathlib.Path(__file__).parent.parent.parent / "app" / "connectors" / "executors" / "oci"
...
def test_rollback_capability_declared(name):
    mod = importlib.import_module(f"app.connectors.executors.oci.{name}")
