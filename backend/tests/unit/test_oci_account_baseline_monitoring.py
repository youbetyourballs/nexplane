...
async def test_mock_mode_returns_structure():
    from app.connectors.executors.oci.oci_account_baseline_monitoring import execute
    connector = MagicMock()
    connector.credentials = {}
    result = await execute({}, [], connector)
    assert result["mock"] is True
    assert result["promote_to"] == "oci_account_baseline_hardening"
...
async def test_rollback_capability():
    from app.connectors.executors.oci.oci_account_baseline_monitoring import ROLLBACK_CAPABILITY
    assert ROLLBACK_CAPABILITY == "full"


@pytest.mark.asyncio
async def test_rollback_mock_no_newly_enabled():
    from app.connectors.executors.oci.oci_account_baseline_monitoring import rollback
    connector = MagicMock()
    connector.credentials = {}
