...
for pkg in [
    ...
    "oci", "ldap3", "winrm", "kubernetes", "kubernetes.client",
    "kubernetes.config", "requests", "httpx",
]:
    parts = pkg.split(".")
...
EXECUTORS_DIR = pathlib.Path(__file__).parent.parent.parent / "app" / "connectors" / "executors" / "kubernetes"
...
def test_rollback_capability_declared(name):
    mod = importlib.import_module(f"app.connectors.executors.kubernetes.{name}")
