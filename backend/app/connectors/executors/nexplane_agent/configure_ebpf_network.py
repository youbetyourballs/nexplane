...
async def execute(parameters: dict, asset_ids: list, connector) -> dict:
    ...
    # non-fatal: a RuntimeError just means the agent is unreachable for the probe,
    # which will surface clearly in the main job below.
    diagnostics: dict = {
        "kernel_version": None,
    ...
    try:
        ...
        logger.info(
            "eBPF preflight diagnostics: kernel=%s bpf_syscall=%s cap_bpf=%s "
        ...
        logger.warning("eBPF preflight probe failed (non-fatal): %s", diag_err)
