...
async def _capture_pre_state(connector, service_name: str) -> dict:
    stdout, _, _ = await _run(connector, f"systemctl is-active {service_name} 2>/dev/null || true")
    was_active = stdout.strip() == "active"
    stdout, _, _ = await _run(connector, f"systemctl is-enabled {service_name} 2>/dev/null || true")
...
async def _docker_available(connector) -> bool:
    _, _, rc = await _run(connector, "docker info >/dev/null 2>&1")
    return rc == 0
...
async def _build_image(connector, service_name: str, registry: str) -> dict:
    ...
    await _run(connector, f"docker build -f /tmp/nexplane-Dockerfile -t {image_tag} /")
    stdout, _, _ = await _run(connector, f"docker inspect --format='{{{{.Id}}}}' {image_tag}")
    image_digest = stdout.strip()
    return {"image_name": image_tag, "image_digest": image_digest}


async def execute(parameters: dict, asset_ids: list, connector) -> dict:
    if dispatch_agent_job is None:
        raise RuntimeError("dispatch_agent_job not available")
    ...
    await _run(connector, f"docker push {image_name}")
...
async def rollback(parameters: dict, execution_result: dict, connector) -> dict:
    ...
    if path == "inplace":
        ...
        if container_id:
            await _run(connector, f"docker stop {container_id} 2>/dev/null || true")
            await _run(connector, f"docker rm {container_id} 2>/dev/null || true")
        if pre_state.get("was_active"):
            await _run(connector, f"systemctl start {service_name}")
        if pre_state.get("was_enabled"):
            await _run(connector, f"systemctl enable {service_name} 2>/dev/null || true")
    ...
    if path == "remote":
        ...
        try:
            await dispatch_agent_job(
                command="containerize_workload_rollback",
                parameters={"app_name": service_name, "namespace": namespace},
                asset_ids=[str(target_cluster_id)] if target_cluster_id else [],
                timeout_seconds=60,
            )
        except Exception as e:
            logger.error(f"Rollback dispatch failed: {e}")
            rolled_back = False
        return {"rolled_back": rolled_back, "path": "remote", "note": "source service was not stopped"}
