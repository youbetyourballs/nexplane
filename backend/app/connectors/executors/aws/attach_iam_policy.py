...
async def _real_execute(creds: dict, parameters: dict) -> dict:
    ...
    def _call():
        if principal_type == 'user':
            ...
            iam.attach_group_policy(GroupName=principal_name, PolicyArn=policy_arn)
    ...
    return {"action": "attach_iam_policy", "principal_type": principal_type, "principal_name": principal_name, "policy_arn": policy_arn, "executed_at": datetime.now(timezone.utc).isoformat()}


async def execute(parameters: dict, asset_ids: list, connector) -> dict:
    creds = getattr(connector, 'credentials', {})
    if not creds:
        return {"action": "attach_iam_policy", "principal_type": parameters.get('principal_type'), "principal_name": parameters.get('principal_name'), "policy_arn": parameters.get('policy_arn'), "mock": True}
