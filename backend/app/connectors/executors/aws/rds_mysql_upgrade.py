...
"""AWS RDS MySQL instance version upgrade executor.

Triggers a major or minor version upgrade on an RDS MySQL instance.
...
async def execute(parameters: dict, asset_ids: list, connector) -> dict:
    ...
    await _run(_modify)
    logger.info("rds_mysql_upgrade: upgrade to %s initiated for %s", target_version, db_id)
    ...
    while time.time() < deadline:
        ...
        logger.info("rds_mysql_upgrade: polling status=%s version=%s", status, ver)
        if status == "available" and ver == target_version:
            final_version = ver
            break
    else:
        raise TimeoutError(
            f"RDS MySQL {db_id} did not reach {target_version} within {_POLL_TIMEOUT}s"
        )
...
async def rollback(parameters: dict, execution_result: dict, connector) -> dict:
    ...
    return {
        "rolled_back": False,
        "reason": (
            f"RDS MySQL does not support engine version downgrades. "
