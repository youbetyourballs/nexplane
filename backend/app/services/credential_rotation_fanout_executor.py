...
async def _update_consumer(consumer: dict, new_value: str, db, connector_cache: dict) -> dict:
    ...
    # Remap generic keys to surface-specific names expected by update functions
    if surface == "k8s_configmap":
        if "name" in params and "configmap_name" not in params:
            params["configmap_name"] = params.pop("name")
        if "data_key" in params and "key" not in params:
            params["key"] = params.pop("data_key")
