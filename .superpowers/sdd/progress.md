...
- OS Upgrade Task 1: complete (commits cbe1534..995ec8a, 5/5 tests on EC2, review clean after preflight-skip fix)
- OS Upgrade Task 2: complete (commit f19e14c, 2 skips on EC2, review clean)
## OS Upgrade plan COMPLETE
- Final review fixes (C2/C3): complete (commit bff87d8 — run_in_executor for all boto3 calls, snapshot_completed waiter before create_volume)
## BOTH PLANS COMPLETE — all 5 tasks done, final review approved
## Credential Rotation Plan (2026-07-15-credential-rotation.md)
Base commit: 3d6467a
- Task 1: complete (commit 69e0860, review clean — credential_rotation ChangeType + paused ChangeRequestStatus + cred001 migration)
- Task 2: complete (commits 5789954..aa5d6e8, review clean after unused import fix — credential_rotation planning handler, 3/3 tests)
- Task 3: complete (commits aa5d6e8..447688e, review clean — execute_steps + execute_filo_rollback + _init_steps + _resolve_connector, 2/2 tests. Minor: M1 asset_ids=[] is correct (4th positional arg to execute_action), M2 all([]) vacuous truth acceptable, M3 no filo rollback test (deferred), M4 _attach_credentials private import)
- Task 4: complete (commit c3d91b5, review clean — credential_rotation branch in activities.py, paused/completed early exit in execute_change_workflow.py, backend imports OK)
- Task 5: complete (commits 314073b..5f7a444, review clean after role guard fix — retry-step + skip-step endpoints, rollback allows paused status, admin/approver guards on new endpoints)
Task 6: complete (commits ad9efee..2570f5a, review clean after tautology fix)
Task 7: complete (commits 3111188..a3fa0f8, review clean after final fixes)
Final whole-branch review: clean (2 minor nits, no blockers)
## Plan: Certificate Rotation
- Task 1: complete (commit f1dd8bb, review clean — ChangeType.certificate_rotation + cert001 migration applied on EC2)
- Task 2: complete (commits ca04947..7bbca80, review clean after fix — field validation + preflight_checks for step_ca connector, 6/6 tests)
- Task 3: complete (commits 2b14fea..2bd1c15, review clean after fix — executor phases 1-3, SAN list support, k8s_secret/aws_secret AssetType, migrations cert002+cert003 applied)
- Task 4: complete (commits 709e6c4..3c1732b, review clean after fix — phases 4-5 + execute_certificate_rotation, phase 4 idempotency + unconditional pause check)
- Task 5: complete (commit 6cb69ce, review clean — FILO rollback + strategy C, minor snapshot_key fallback noted)
- Task 6: complete (commit 195e5fb, review clean — certificate_rotation wired into activities, workflow, rollback_executor)
- Task 7: complete (commit 88055d9, review clean — retry-verify + skip-verify endpoints, FOR UPDATE, 202 status)
- Task 8: complete (commits 70220dc..9dee5af, review clean after fix — auto-trigger in expiry worker, submit-for-approval side effects replicated)
- Task 9: complete (commits aeb66cf..cde5559, review clean — 3 passed / 1 skip / 1 xfail on EC2; Phase 5 xfail on step-ca v0.27 CLI limitation)
- Final whole-branch review: APPROVED (commit ea8f399, 4 fixes applied)

## Certificate Rotation Plan — COMPLETE (2026-07-18)
- Task 1: complete (cert001_certificate_rotation migration, ChangeType.certificate_rotation enum)
- Task 2: complete (planning engine validation, 6/6 unit tests)
- Task 3: complete (executor helpers + phases 1-3: scan, snapshot, rotate)
- Task 4: complete (executor phases 4-5: update, verify, execute_certificate_rotation)
- Task 5: complete (FILO rollback with strategy C: restore vs re-issue)
- Task 6: complete (workflow, activities, rollback_executor wiring)
- Task 7: complete (retry-verify and skip-verify API endpoints)
- Task 8: complete (auto-trigger in credential_expiry_worker)
- Task 9: complete (5-phase smoke: 3 passed, 1 skipped no-k8s, 1 xfail step-ca no cert listing)

## Plan: MCP Smoke Tests (2026-07-20)
- Task 1: complete (commits f55f79e..3b3e0cb, review clean — check_for_budget_pause() added, import verified on EC2)
- Task 2: complete (no new commits — 21/21 phases passed on EC2 on first run, no fixes needed)
